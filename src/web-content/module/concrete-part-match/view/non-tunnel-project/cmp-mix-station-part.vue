<template>
  <mu-v-box size="1"
    ...
    <mu-flex-box size="1"
      style="padding: 8px">
      <div id="mixStationPart" style="width: 100%;height:100%" />
    </mu-flex-box>
    <mu-flex-box
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async queryData () {
        const fn1 = this.$http.post('services/non-tunnel/machine-parts', this.setParams())
        const fn2 = this.$http.get('services/non-tunnel/machine-parts/spec-model')
      ...
      async setLinkedFn () {
        ...
        const concretePartIds = []
        // 已经关联,但是关联了其他算量部位
        this.checkArr.forEach(item => {
          const isSameSqPart = partId === item.refProjectEntryWorkId && specModel === item.refSpecModel
          if (item.isRelated && !isSameSqPart) {
            cancelRelatedRefIds.push(item.recordRefId)
            concretePartIds.push(item.partId)
          }
          if (!item.isRelated) {
            concretePartIds.push(item.partId)
          }
        ...
        if (cancelRelatedRefIds.length > 0) {
          result = await this.$http.patch('services/non-tunnel/machine-parts/cancel-related', { relatedRefIds: cancelRelatedRefIds })
        }
        if (concretePartIds.length > 0) {
          const options = {
            partId,
            specModel,
            isNewPart: partType > 0,
            concretePartIds
          }
          if (projectUnitWorkId) Object.assign(options, { projectUnitWorkId })
          if (entryWorkId) Object.assign(options, { entryWorkId })
          result = await this.$http.patch('services/non-tunnel/machine-parts/related', options)
        }
        if (!result) {
          return
        }
      ...
      async cancelLinkedFn () {
        ...
        const relatedRefIds = filters.map(item => {
          return item.recordRefId
        })
        const result = await this.$http.patch('services/non-tunnel/machine-parts/cancel-related', { relatedRefIds })
        if (!result) return
        notify('success', '取消成功')
        this.$emit('changeLinked', true)
        if (this.type === 3) {
        ...
        if (this.type) {
          ...
          if (this.type === 3) {
            ...
            Object.assign(params, {
              projectEntryWorkId: partId,
              partSpecModel: specModel
            })
          }
        }
